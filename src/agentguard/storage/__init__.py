...
def _default_db_path() -> Path:
    env = os.environ.get("AGENTGUARD_DB_PATH")
    if env:
        return Path(env)
    return Path.home() / ".agentguard" / "agentguard.db"


class AgentGuardDB:
    """Central database access for all agentguard persistent data."""
    ...
    def __enter__(self) -> "AgentGuardDB":
        self.connect()
        return self
...
__all__ = ["CURRENT_VERSION", "AgentGuardDB"]
