...
def _show_content_security_grade(content: str) -> None:
    ...
    from agentguard.security.scanner import ScanReport as _ScanReport
    from agentguard.security.scanner import scan_content, score_report
    ...
    if findings:
        console.print(
            ...
            " Use [bold]agentguard scan --fix[/bold] after writing.[/dim]"
        )
...
def _write_harness_files(
    ...
    """
    from agentguard.core.parsers import AssetType as _AssetType
    from agentguard.harness import HarnessFormat, adapt
    from agentguard.llm.parser import GeneratedAsset, parse_llm_response
...
def _show_suggestion(identifier: str) -> None:
    ...
    """
    from agentguard.evaluation.suggest import suggest_for_repo
...
def suggest(repo: Path, apply: bool, dry_run: bool) -> None:
    ...
    console.print("\nUse `agentguard show --suggestion <N>` for draft content.")
...
def profile(repo: Path) -> None:
    ...
    from agentguard.telemetry.profiler import profile_repo, save_workflow_model
    ...
    if result.session_count == 0:
        ...
        console.print("Install hooks first: agentguard hooks install")
        return
...
def analyze_cmd(repo: Path) -> None:
    ...
    from agentguard.intelligence.analyzer import analyze_repo
...
def export_cmd(
    ...
    from agentguard.core.catalog import CatalogEntry
    from agentguard.core.parsers import AssetType as _AssetType
    from agentguard.harness import HarnessFormat, adapt
...
def extract_patterns_cmd() -> None:
    ...
    from agentguard.intelligence.patterns import extract_all_patterns
...
def apply_pattern_cmd(pattern_name: str, repo: Path) -> None:
    ...
    from agentguard.intelligence.analyzer import analyze_repo
    from agentguard.intelligence.patterns import PatternTemplate
...
def check_regression_cmd(session_id: str, repo: Path) -> None:
    ...
    from agentguard.evaluation.evaluator import check_regression, log_regression
...
def variant_cmd(asset_id: str, name: str, description: str) -> None:
    ...
    from agentguard.evaluation.evaluator import ABTestStore, create_variant
...
def compare_cmd(asset_a: str, asset_b: str) -> None:
    ...
    from agentguard.evaluation.evaluator import compare_variants
...
def promote_cmd(variant_id: str) -> None:
    ...
    from agentguard.evaluation.evaluator import ABTestStore, promote_variant
...
def rollback_best_cmd(asset_id: str) -> None:
    ...
    from agentguard.evaluation.evaluator import evaluate_asset
    from agentguard.security.snapshots import SnapshotStore
    from agentguard.telemetry.events import find_sessions_dir, parse_all_sessions
...
def schema_show(asset_type: str | None) -> None:
    ...
    from agentguard.intelligence.schema_validator import show_schema
...
def schema_check() -> None:
    ...
    from agentguard.intelligence.schema_validator import check_schemas
...
def schema_update() -> None:
    ...
    from agentguard.intelligence.schema_validator import check_schemas, update_schemas
...
def schema_reset() -> None:
    ...
    from agentguard.intelligence.schema_validator import reset_schemas
