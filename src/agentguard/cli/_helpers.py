...
def _configure_logging(
    ...
    """
    root = logging.getLogger("agentguard")
...
def _load_config() -> AgentGuardConfig:
    """Load the AgentGuard configuration from the default path.

    Returns:
        Loaded configuration with defaults for missing values.
    """
    return AgentGuardConfig.load()


def _load_catalog(config: AgentGuardConfig) -> Catalog:
    """Load the asset catalog from disk.

    Args:
        config: AgentGuard configuration containing catalog path.
