...
from agentguard.core.parsers import AssetType, _split_frontmatter
...
class PatternTemplate(BaseModel):
    ...
    def save(self, output_dir: Path | None = None) -> Path:
        ...
        Args:
            output_dir: Override directory. Defaults to ~/.agentguard/patterns/.

        Returns:
            Path to the saved pattern file.
        """
        dest = output_dir or (Path.home() / ".agentguard" / "patterns")
    ...
    def load_pattern(
        ...
        """
        directory = patterns_dir or (Path.home() / ".agentguard" / "patterns")
