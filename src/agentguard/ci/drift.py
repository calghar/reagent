...
from agentguard.llm.config import PROVIDER_ENV_KEYS
...
def _load_agentguard_config() -> dict[str, object] | None:
    """Load the agentguard config YAML from the default path.

    Returns:
        Parsed YAML dict, or None if the file does not exist or is invalid.
    """
    config_path = Path.home() / ".agentguard" / "config.yaml"
    ...
    try:
        ...
        logger.warning("Failed to parse agentguard config: %s", exc)
    return None


def _get_configured_provider(config: dict[str, object]) -> str | None:
    """Extract the LLM provider name from a agentguard config dict.

    Args:
        config: Parsed agentguard config dictionary.
...
class DriftDetector:
    """Detect drift in agentguard-managed assets within a repository."""
    ...
    def _check_config_drift(self) -> list[DriftReport]:
        """Check if the agentguard config references providers without env vars set.
        ...
        """
        config = _load_agentguard_config()
        if config is None:
            return []
        ...
        return [
            DriftReport(
                asset_path=str(Path.home() / ".agentguard" / "config.yaml"),
                asset_type="config",
