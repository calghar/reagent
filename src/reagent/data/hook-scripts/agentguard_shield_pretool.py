...
``reagent shield check`` whether the tool call is permitted under the
...
pointing Claude Code's settings at it, or use ``reagent shield install``.
"""
...
def main() -> int:
    ...
    from reagent.shield.enforcer import ShieldEnforcer
