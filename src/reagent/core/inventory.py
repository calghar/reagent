...
from reagent.core.catalog import Catalog, CatalogEntry, entry_from_parsed
...
def scan_all(config: ReagentConfig) -> list[CatalogEntry]:
    """Scan all configured root directories for repos with .claude/ dirs.

    Args:
        config: Reagent configuration with scan roots and options.
...
def run_inventory(config: ReagentConfig, catalog: Catalog) -> tuple[int, int, int]:
    """Run a full inventory scan and update the catalog.

    Args:
        config: Reagent configuration with scan roots.
