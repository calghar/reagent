...
BUNDLED_SCHEMAS = importlib.resources.files("reagent.data.schemas.claude-code")
SCHEMAS_DIR = Path.home() / ".reagent" / "schemas" / "claude-code"
...
def _ensure_schemas_installed() -> None:
    """Copy bundled schemas to ~/.reagent/schemas/ if not present."""
...
def reset_schemas(target_dir: Path | None = None) -> Path:
    ...
    Args:
        target_dir: Override directory. Defaults to ~/.reagent/schemas/claude-code/.
...
def _load_schema(schema_name: str, schema_dir: Path | None = None) -> dict[str, Any]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f'Schema file "{schema_name}" is missing. Run "reagent schema reset" '
    ...
    try:
        ...
        raise json.JSONDecodeError(
            f'Schema file "{schema_name}" is corrupt. Run "reagent schema reset" '
